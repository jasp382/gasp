...
def geovideos_to_array(search_words, epsg_out=4326,
    ...
    import os
    from gasp3.pyt.oss         import del_files_by_name
    from gasp3.gt.to.geom      import new_pnt
    from gasp3.gt.prj          import proj
    from gasp3.gt.anls.tplgy   import point_in_polygon
    from gasp3.gt.anls.prox.bf import draw_buffer
    ...
    if spatial_filter:
        ...
        bufferCenter = proj(new_pnt(x_center, y_center), None,
            3857, inEPSG=4326, api='OGRGeom'
        )
    ...
    for instance in videos_with_location:
        # Create point
        WGS_POINT = new_pnt(float(instance['x']), float(instance['y']))
        
        point = proj(WGS_POINT, None, 3857, inEPSG=4326, gisApi='OGRGeom')
        ...
        if isPointInPolygon:
            if epsg_out != 4326:
                trans_point = proj(
                    WGS_POINT, None, epsg_out, inEPSG=4326, gisApi='OGRGeom')
