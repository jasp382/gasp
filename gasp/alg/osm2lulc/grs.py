...
def raster_based(osmdata, nomenclature, refRaster, lulcRst,
    ...
    # Gasp dependencies #
    # ************************************************************************ #
    from gasp.pyt.oss                import create_folder, get_filename
    ...
    if roadsAPI == 'POSTGIS':
        ...
        from gasp.alg.osm2lulc.mod2  import roads_sqdb
    else:
        from gasp.alg.osm2lulc.utils import osm_to_sqdb
        from gasp.alg.osm2lulc.mod2  import grs_rst_roads
    from gasp.alg.osm2lulc.utils     import osm_project, add_lulc_to_osmfeat, osmlulc_rsttbl
    from gasp.alg.osm2lulc.utils     import get_ref_raster
    from gasp.alg.osm2lulc.mod1      import grs_rst
    from gasp.alg.osm2lulc.m3_4      import rst_area
    from gasp.alg.osm2lulc.mod5      import basic_buffer
    from gasp.alg.osm2lulc.mod6      import rst_pnt_to_build
    # ************************************************************************ #
    # Global Settings #
    # ************************************************************************ #
    ...
    from gasp.alg.osm2lulc.var import PRIORITIES, osmTableData, LEGEND
    ...
    from gasp.gt.nop.mos  import mosaic_raster
...
def vector_based(osmdata, nomenclature, refRaster, lulcShp,
    ...
    * POSTGIS
    ...
    # GASP dependencies #
    # ************************************************************************ #
    from gasp.pyt.oss                import get_filename, get_fileformat, create_folder
    from gasp.gt.wenv.grs            import run_grass
    if RoadsAPI == 'POSTGIS':
        from gasp.sql.db             import create_db
        from gasp.sql.to             import osm_to_pgsql
        ...
        from gasp.alg.osm2lulc.utils import osm_to_sqdb
    from gasp.alg.osm2lulc.utils     import osm_project, add_lulc_to_osmfeat, get_ref_raster
    from gasp.gt.to.shp              import shps_to_shp
    from gasp.alg.osm2lulc.mod1      import grs_vector
    if RoadsAPI == 'SQLITE' or RoadsAPI == 'POSTGIS':
        from gasp.alg.osm2lulc.mod2  import roads_sqdb
    else:
        from gasp.alg.osm2lulc.mod2  import grs_vec_roads
    from gasp.alg.osm2lulc.m3_4      import grs_vect_selbyarea
    from gasp.alg.osm2lulc.mod5      import grs_vect_bbuffer
    from gasp.alg.osm2lulc.mod6      import vector_assign_pntags_to_build
    # ************************************************************************ #
    # Global Settings #
    # ************************************************************************ #
    # Check if input parameters exists!
    ...
    from gasp.alg.osm2lulc.var import osmTableData, PRIORITIES, LEGEND
    ...
    # IMPORT SOME GASP MODULES FOR GRASS GIS #
    # ************************************************************************ #
    from gasp.gt.anls.ovlay import erase
    from gasp.gt.wenv.grs   import rst_to_region
    ...
    from gasp.gt.to.rst     import rst_to_grs
    # ************************************************************************ #
    # SET GRASS GIS LOCATION EXTENT #
    # ************************************************************************ #
    extRst = rst_to_grs(refRaster, 'extent_raster')
    ...
    """
    from gasp.gt.to.shp import same_attr_to_shp
